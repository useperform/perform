...
class ResetTokenManager
{
    ...
    {
        $user = $this->em->getRepository('PerformBaseBundle:User')
    ...
    {
        return $this->em->getRepository('PerformBaseBundle:ResetToken')
            ->find($id);
    }
    ...
    {
        $notification = new Notification($token->getUser(), 'PerformBaseBundle:reset_password', [
